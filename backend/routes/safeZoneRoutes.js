...
router.post('/', authenticate, authorize('admin', 'disaster_officer'), async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to create safe zone', details: error.message });
  }
});
...
router.get('/', authenticate, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch safe zones', details: error.message });
  }
});
...
router.get('/:id', authenticate, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch safe zone', details: error.message });
  }
});
...
router.put('/:id', authenticate, authorize('admin', 'disaster_officer'), async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to update safe zone', details: error.message });
  }
});
...
router.delete('/:id', authenticate, authorize('admin'), async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to delete safe zone', details: error.message });
  }
});
...
router.post('/check-location', authenticate, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Location check failed', details: error.message });
  }
});
