...
router.post('/allocate', authenticate, authorize('admin', 'disaster_officer'), async (req, res) => {
  try {
    const { resource_id, quantity } = req.body;
    const resource = await Resource.findById(resource_id);
    if (!resource) return res.status(404).json({ error: 'Resource not found' });
    if (resource.available_quantity < quantity) {
      return res.status(400).json({ error: 'Insufficient stock', available: resource.available_quantity });
    }
    ...
    res.status(500).json({ error: 'Failed to allocate', details: error.message });
  }
});
