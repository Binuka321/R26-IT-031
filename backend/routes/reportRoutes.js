...
router.get('/resources', authenticate, async (req, res) => {
  try {
    ...
    const report = resources.map(r => ({
      ...
      available: r.available_quantity, unit: r.unit,
...
router.get('/dashboard', authenticate, async (req, res) => {
  try {
    ...
    const totalSanitary = resources.filter(r => r.resource_type === 'sanitary').reduce((s, r) => s + r.available_quantity, 0);

    res.json({
      status: 'success',
      data: {
        totalSafeZones, totalCamps, highPriority, medPriority, lowPriority,
        totalPopulation: totalPop, totalDistributions: totalDist,
        pendingDistributions: pendingDist, completedDistributions: deliveredDist,
        totalFood, totalWater, totalMedicine, totalSanitary
      }
    });
  } catch (error) {
