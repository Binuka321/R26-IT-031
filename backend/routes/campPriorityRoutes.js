...
const buildItemPriorityData = (camp, result) => {
  ...
  return {
    ...
    medicine_priority: result.medicine_priority,
    sanitary_priority: result.sanitary_priority,
    recommended_food_qty: Math.max(0, Math.round((camp.population || 0) * 6 - (camp.food_available || 0))),
    recommended_water_qty: Math.max(0, Math.round((camp.population || 0) * 10 - (camp.water_available || 0))),
    recommended_medicine_qty: Math.max(0, Math.round((camp.population || 0) * 0.5 - (camp.medicine_available || 0))),
    recommended_sanitary_qty: Math.max(0, Math.round((camp.population || 0) * 2 - (camp.sanitary_available || 0))),
  ...
  async (req, res) => {
    try {
      ...
      await ItemPriority.findOneAndUpdate(
        { camp_id: camp._id },
        buildItemPriorityData(camp, result),
        { upsert: true, new: true },
      );
