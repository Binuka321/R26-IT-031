...
router.post('/', authenticate, async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to create disease result', details: error.message });
  }
});

router.get('/', authenticate, async (req, res) => {
  try {
    const results = await DiseaseResult.find().populate('camp_id', 'camp_name').sort({ detected_date: -1 });
...
router.get('/camp/:campId', authenticate, async (req, res) => {
  try {
    const results = await DiseaseResult.find({ camp_id: req.params.campId }).sort({ detected_date: -1 });
    res.json({ status: 'success', data: results });
  } catch (error) {
    res.status(500).json({ error: 'Failed to fetch', details: error.message });
  }
...
router.get('/alerts', authenticate, async (req, res) => {
  try {
    const alerts = await DiseaseResult.find({ risk_level: 'High', status: 'Active' }).populate('camp_id', 'camp_name population');
