...
export const StatCard: React.FC<{
  ...
  const colors: Record<string, { border: string; bg: string; iconBg: string; iconText: string }> = {
    cyan: { border: 'border-cyan-500', bg: 'from-cyan-50 to-blue-50', iconBg: 'bg-cyan-100', iconText: 'text-cyan-600' },
    purple: { border: 'border-purple-500', bg: 'from-purple-50 to-indigo-50', iconBg: 'bg-purple-100', iconText: 'text-purple-600' },
    emerald: { border: 'border-emerald-500', bg: 'from-emerald-50 to-green-50', iconBg: 'bg-emerald-100', iconText: 'text-emerald-600' },
    amber: { border: 'border-amber-500', bg: 'from-amber-50 to-yellow-50', iconBg: 'bg-amber-100', iconText: 'text-amber-600' },
    rose: { border: 'border-rose-500', bg: 'from-rose-50 to-pink-50', iconBg: 'bg-rose-100', iconText: 'text-rose-600' },
    blue: { border: 'border-blue-500', bg: 'from-blue-50 to-indigo-50', iconBg: 'bg-blue-100', iconText: 'text-blue-600' },
    indigo: { border: 'border-indigo-500', bg: 'from-indigo-50 to-violet-50', iconBg: 'bg-indigo-100', iconText: 'text-indigo-600' },
  };
  const c = colors[color];
  return (
    <div className={`rounded-2xl p-5 border-l-4 ${c.border} bg-gradient-to-r ${c.bg} shadow-lg hover:shadow-xl transition-shadow duration-300`}>
...
export const PriorityBadge: React.FC<{ level: string }> = ({ level }) => {
  ...
  return (
    <span className={`px-3 py-1 rounded-full text-xs font-bold inline-flex items-center gap-1 ${config[level] || 'bg-gray-200 text-gray-600'}`}>
      <span className="material-icons text-xs">{level === 'High' ? 'warning' : level === 'Medium' ? 'priority_high' : 'check_circle'}</span>
      {level}
    </span>
  );
};
...
export const StatusBadge: React.FC<{ status: string }> = ({ status }) => {
  const config: Record<string, { cls: string; icon: string }> = {
    'Pending': { cls: 'bg-amber-100 text-amber-800 border border-amber-200', icon: 'schedule' },
    'On the Way': { cls: 'bg-blue-100 text-blue-800 border border-blue-200', icon: 'local_shipping' },
    'Delivered': { cls: 'bg-emerald-100 text-emerald-800 border border-emerald-200', icon: 'check_circle' },
    'Failed': { cls: 'bg-rose-100 text-rose-800 border border-rose-200', icon: 'cancel' },
    'Active': { cls: 'bg-emerald-100 text-emerald-800 border border-emerald-200', icon: 'check_circle' },
    'Inactive': { cls: 'bg-gray-100 text-gray-600 border border-gray-200', icon: 'pause_circle' },
    'Safe': { cls: 'bg-emerald-100 text-emerald-800 border border-emerald-200', icon: 'verified_user' },
    'At Risk': { cls: 'bg-amber-100 text-amber-800 border border-amber-200', icon: 'warning' },
    'Compromised': { cls: 'bg-rose-100 text-rose-800 border border-rose-200', icon: 'dangerous' },
  };
  const c = config[status] || { cls: 'bg-gray-100 text-gray-600', icon: 'info' };
  return (
    <span className={`px-3 py-1 rounded-full text-xs font-medium inline-flex items-center gap-1 ${c.cls}`}>
...
export const SearchFilter: React.FC<{
  searchTerm: string; onSearch: (v: string) => void;
...
export const PageHeader: React.FC<{
  ...
  <div className="flex flex-col md:flex-row md:items-center justify-between mb-6">
    ...
    {actions && <div className="mt-3 md:mt-0 flex flex-wrap gap-2">{actions}</div>}
  </div>
);
...
export const PrimaryButton: React.FC<{
  ...
  icon?: string; disabled?: boolean; className?: string;
...
export const FormInput: React.FC<{
  ...
  type?: string; placeholder?: string; required?: boolean; min?: number;
}> = ({ label, value, onChange, type = 'text', placeholder, required, min }) => (
  <div>
    <label className="block text-sm font-medium text-gray-700 mb-1">{label}{required && <span className="text-rose-500">*</span>}</label>
